...
public static class SeedData
{
    ...
    {
        var meatCategory = await ctx.RecipeCategories.FirstOrDefaultAsync(c => c.RecipeCategoryId == 1) ?? ctx
            .RecipeCategories.Add(new RecipeCategory
            {
                RecipeCategoryId = 1,
                Name = "Meat"
            }).Entity;
        var vegetarianCategory = await ctx.RecipeCategories.FirstOrDefaultAsync(c => c.RecipeCategoryId == 2) ?? ctx
            .RecipeCategories.Add(new RecipeCategory
            {
                RecipeCategoryId = 2,
                Name = "Vegetarian"
            }).Entity;
        var fishCategory = await ctx.RecipeCategories.FirstOrDefaultAsync(c => c.RecipeCategoryId == 3) ?? ctx
            .RecipeCategories.Add(new RecipeCategory
            {
                RecipeCategoryId = 3,
                Name = "Fish"
            }).Entity;
        var soupsCategory = await ctx.RecipeCategories.FirstOrDefaultAsync(c => c.RecipeCategoryId == 4) ?? ctx
            .RecipeCategories.Add(new RecipeCategory
            {
                RecipeCategoryId = 4,
                Name = "Soups"
            }).Entity;
        var specialCategory = await ctx.RecipeCategories.FirstOrDefaultAsync(c => c.RecipeCategoryId == 5) ?? ctx
            .RecipeCategories.Add(new RecipeCategory
            {
                RecipeCategoryId = 5,
                Name = "Special"
            }).Entity;
        var snacksCategory = await ctx.RecipeCategories.FirstOrDefaultAsync(c => c.RecipeCategoryId == 6) ?? ctx
            .RecipeCategories.Add(new RecipeCategory
            {
                RecipeCategoryId = 6,
                Name = "Snacks"
            }).Entity;
        var sideDishesCategory = await ctx.RecipeCategories.FirstOrDefaultAsync(c => c.RecipeCategoryId == 7) ?? ctx
            .RecipeCategories.Add(new RecipeCategory
            {
                RecipeCategoryId = 7,
                Name = "Side Dishes"
        ...
        _ = await ctx.Recipes.FirstOrDefaultAsync(r => r.RecipeId == 2) ?? ctx.Recipes.Add(new Recipe
        {
            ...
            RecipeId = 2,
            Ingredients =
                "1 1/2 cups of berries (black or blue)\n2 tablespoons flour\n1/8 teaspoons salt\n1/2 cups sugar\n1 tablespoon of lemon juice",
            Status = Status.Approved,
            Method =
        ...
        _ = await ctx.Recipes.FirstOrDefaultAsync(r => r.RecipeId == 3) ?? ctx.Recipes.Add(new Recipe
        {
            ...
            Status = Status.Approved,
            Method =
                "Place the butter in a frying pan, and when the pan is hot and the butter is melted, add the beef\nAllow it to frizzle\n" +
        ...
        _ = await ctx.Recipes.FirstOrDefaultAsync(r => r.RecipeId == 16) ?? ctx.Recipes.Add(new Recipe
        {
            ...
            Status = Status.Approved,
            Method =
                "Place alternate layers of diced cooked potatoes and sliced hard-cooked eggs in a baking dish.\nSeason.\nPour a thin white sauce over all of this.\nPlace in a moderate oven fifteen \nminutes.",
            Categories = new List<RecipeCategory> { vegetarianCategory, sideDishesCategory },
            Notes = "",
            Serves = "4",
